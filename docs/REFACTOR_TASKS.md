...
| P1.22 | Remove `GreaseweazleProviderV2::raw_handle()` + `HardwareTab::gwDevice()`  | P1.20, P1.21 | 🔄 **entsperrt** — nach MF-200/MF-201 haben `raw_handle()`/`gwDevice()` *null* echte Consumer (verifiziert via `git grep`); `gwDevice()` ist ein toter public Accessor. Bereit zum Entfernen. |
