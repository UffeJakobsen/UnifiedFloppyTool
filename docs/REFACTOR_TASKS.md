...
| P1.20 | Migrate `FluxCaptureJob` to V2 outcome surface (drop uft_gw_* direct path) | P1.18, **P0.x** | 🛑 **BLOCKED — STOP-Befund (2026-05-14)**. (1) `FluxCaptured` (`outcomes.h:179`) trägt KEINE Per-Revolution-Index-Grenzen — nur flaches `transitions_ns` + `int revolutions`-Zähler. `FluxCaptureJob` splittet Flux pro Revolution über `flux->index_times[]` für `scp_writer_add_track` — die V2-Oberfläche kann das nicht ausdrücken; Migration as-is würde Index-Grenzen verlieren → F-3-Verstoß. Sauberer Fix = Feld zu `FluxCaptured` hinzufügen = **geschützter Foundation-Header** → Mensch/`type-system-architect`-Entscheidung nötig. (2) Kein `select_drive`/`select_head` auf der V2-Oberfläche. (3) Forensik-Befund: `do_read_raw_flux` (`greaseweazle_provider_v2.cpp:246-264`) verwirft `flux->index_times[]`, obwohl der `.h`-Docstring (`:24-26`) F-3-Konformität *behauptet*. |
| P1.21 | Migrate `FluxWriteJob`   to V2 outcome surface (drop uft_gw_* direct path) | P1.18      | 🛑 BLOCKED — gleicher `select_drive`/`select_head`-Gap wie P1.20 (2). |
| P1.22 | Remove `GreaseweazleProviderV2::raw_handle()` + `HardwareTab::gwDevice()`  | P1.20, P1.21 | ⬜ — blockiert via P1.20/P1.21. Der `raw_handle()`-Escape-Hatch existiert genau weil die V2-Oberfläche heute nicht ausdrücken kann was die Jobs brauchen. |
